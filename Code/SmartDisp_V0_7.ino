...
void set_Pixel_Color(bool copyBlack, uint16_t n, uint8_t r, uint8_t g,
                     uint8_t b) {
    if (copyBlack) {
...
void rowzero(bool copyBlack) {
    int startpix;
    startpix = 0;
    set_Pixel_Color(copyBlack, startpix, matrix[0][3].r, matrix[0][3].g,
                    matrix[0][3].b);
}
...
void rowone(bool copyBlack) {
    ...
    startpix = 1;
    for (int i = 0; i <= 4; i++) {
        set_Pixel_Color(copyBlack, startpix + i, matrix[1][5 - i].r,
                        matrix[1][5 - i].g, matrix[1][5 - i].b);
    }
}
...
void lastrow(bool copyBlack) {
    ...
    startpix = 125;
    for (int i = 0; i <= 2; i++) {
        set_Pixel_Color(copyBlack, startpix + i, matrix[19][4 - i].r,
                        matrix[19][4 - i].g, matrix[19][4 - i].b);
    }
}
...
void copyrow(short rownmb, bool copyBlack) {
    ...
    for (int i = 0; i <= 6; i++) {
        set_Pixel_Color(copyBlack, startpix + i, matrix[rownmb][i].r,
                        matrix[rownmb][i].g, matrix[rownmb][i].b);
    }
}
...
void copyinvrow(short rownmb, bool copyBlack) {
    ...
    for (int i = 0; i <= 6; i++) {
        set_Pixel_Color(copyBlack, startpix + i, matrix[rownmb][6 - i].r,
                        matrix[rownmb][6 - i].g, matrix[rownmb][6 - i].b);
    }
}
...
// each) 0 - 4; 4 is only for ":"
void setDigit(int offset, int select, unsigned char r, unsigned char g,
              unsigned char b) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int j = 0; j < 10; j++) {
          if (i == 2)
          {
            if (j== 0)
            {
              setDigit(1, 4, CR, CG, CB);
              setDigit(2, 9, CR, CG, CB);
            }
            else
            {
              setDigit(2, j-1, CR, CG, CB);
            }
            setDigit(1, j, 255, 0, 0);
          }
          else
          {
            if (j == 0)
            {
              setDigit(1, j, CR, CG, CB);
            }
          }
